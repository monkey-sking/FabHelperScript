...
流水线在**动手领取之前**判一次登录态（`deps.isLoggedIn`），未登录就停在
DONE，商品原样留在待领队列。

这不是可有可无的校验。实测（真实未登录会话）：

| 判据 | 未登录时的值 |
| --- | --- |
| `fab_csrftoken` cookie | **存在**（不能作为已登录的证据） |
| `/i/csrf` | **200**（同样是误信号） |
| `window._epicAccountId` | `''` |
| SSR `/i/users/me` | `isAnonymous: true` |
| `/i/users/me` | 401 |

未登录的详情页上只有「立即购买 / 添加至购物车」，**没有领取按钮**，所以领取
必然失败；而失败一旦写进事件日志就被标成终态，整份免费列表一次性定型 ——
之后就算登录了，这些 uid 也不会再回到待领队列。因此闸门必须拦在动手之前，
而不是等失败发生后再处理。

枚举不受影响：`/i/listings/search` 匿名也返回 200，是只读的，可以照常跑完。
被拦下时调度器会明确报出「未登录」而不是让用户只看到「本程结束」。

登录判据本身见 `Utils.checkAuthentication()` —— 它先读页面内嵌信号
（`_epicAccountId` 与 SSR 的 `isAnonymous` / result UUID），拿不到才退回 cookie。
只看 cookie 会把未登录判成已登录，这是旧实现的一个真实缺陷。
