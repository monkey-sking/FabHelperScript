...
(function () {
    ...
    window.addEventListener('beforeunload', () => {
        ...
        Utils.cleanup();
              ...
              // [OPTIMIZATION_2023_07] 商品卡片请求频率控制优化
      // 确保商品卡片加载请求间隔不低于15秒，减少限速风险，并防止重复请求
      const REQUEST_THROTTLE_CONFIG = {
        minInterval: 15000,      // 最小请求间隔15秒
        enabled: true,           // 默认启用
        version: '1.0.1',        // 优化版本号(添加去重功能)
        lastRequestTime: 0,      // 上次请求时间
        pendingRequests: [],     // 等待中的请求队列
        processing: false,       // 是否正在处理队列
        mergedRequests: 0,       // 已合并的重复请求数
    };
    ...
    const RequestThrottler = {
             // 日志记录，带标记
       logThrottle(message) {
         console.log(`[请求优化] ${message} [OPTIMIZATION_2023_07]`);
         
         // 如果消息包含"请求URL已存在"，更新合并计数
         if (message.includes('请求URL已存在')) {
           REQUEST_THROTTLE_CONFIG.mergedRequests++;
           // 每5次合并打印一次统计
           if (REQUEST_THROTTLE_CONFIG.mergedRequests % 5 === 0) {
             console.log(`[请求优化] 📊 已合并 ${REQUEST_THROTTLE_CONFIG.mergedRequests} 个重复请求，有效避免队列堆积 [OPTIMIZATION_2023_07]`);
           }
         }
       },
             ...
             // 检查是否应该节流请求
       shouldThrottle(url) {
         if (!REQUEST_THROTTLE_CONFIG.enabled) return false;
         
         // 只对商品卡片的加载请求进行节流（通常是带cursor参数的listings搜索请求）
         return url.includes('/i/listings/search') && 
                (url.includes('cursor=') || 
                 !url.includes('aggregate_on=') && 
      ...
      getWaitTime() {
        const now = Date.now();
        const elapsed = now - REQUEST_THROTTLE_CONFIG.lastRequestTime;
        const waitTime = Math.max(0, REQUEST_THROTTLE_CONFIG.minInterval - elapsed);
        return waitTime;
      },
      
      // 处理请求队列
      processQueue() {
        if (REQUEST_THROTTLE_CONFIG.processing || REQUEST_THROTTLE_CONFIG.pendingRequests.length === 0) {
          return;
        }
        
        REQUEST_THROTTLE_CONFIG.processing = true;
        const waitTime = this.getWaitTime();
        
        if (waitTime > 0) {
          this.logThrottle(`请求节流中, 等待 ${waitTime}ms 后继续`);
          ...
          this.executeNextRequest();
             ...
             // 执行队列中的下一个请求
       executeNextRequest() {
         if (REQUEST_THROTTLE_CONFIG.pendingRequests.length === 0) {
           REQUEST_THROTTLE_CONFIG.processing = false;
           return;
         }
         
         const nextRequest = REQUEST_THROTTLE_CONFIG.pendingRequests.shift();
         REQUEST_THROTTLE_CONFIG.lastRequestTime = Date.now();
         
         // 计算请求在队列中的等待时间
         const waitTime = nextRequest.updatedAt ? 
           (Date.now() - nextRequest.addedAt) / 1000 : 0;
         
         this.logThrottle(`⏱️ 执行请求: ${nextRequest.url.substring(0, 50)}... (等待了${waitTime.toFixed(1)}秒)`);
       ...
       enqueueRequest(url, execute) {
         ...
         // 检查队列中是否已有相同URL的请求
         const existingIndex = REQUEST_THROTTLE_CONFIG.pendingRequests.findIndex(
           req => req.url.substring(0, 100) === urlKey
         );
         
         if (existingIndex !== -1) {
           // 如果队列中已有相同URL的请求，替换其执行函数并跳过添加
           this.logThrottle(`🔄 请求URL已存在，更新执行函数: ${url.substring(0, 50)}...`);
           REQUEST_THROTTLE_CONFIG.pendingRequests[existingIndex].execute = execute;
           REQUEST_THROTTLE_CONFIG.pendingRequests[existingIndex].updatedAt = Date.now();
           // 不重复处理队列，因为已经在队列中
           return;
         }
         
         // 新请求，添加到队列
         this.logThrottle(`➕ 请求加入队列: ${url.substring(0, 50)}...`);
         REQUEST_THROTTLE_CONFIG.pendingRequests.push({ 
           url, 
           execute,
           addedAt: Date.now(),
           updatedAt: Date.now()
    ...
    window.XMLHttpRequest.prototype.open = function(method, url, async, user, password) {
      if (RequestThrottler.shouldThrottle(url)) {
        const xhr = this;
        const originalArgs = arguments;
        
        RequestThrottler.enqueueRequest(url, () => {
          originalXHR.apply(xhr, originalArgs);
        });
      } else {
    ...
    // 添加控制接口到全局对象
    if (typeof window.FabHelper === 'undefined') {
      window.FabHelper = {};
    }

    window.FabHelper.requestThrottler = {
      // 启用请求节流
      enable() {
        REQUEST_THROTTLE_CONFIG.enabled = true;
        RequestThrottler.logThrottle('请求频率控制已启用');
      },
      
      // 禁用请求节流
      disable() {
        REQUEST_THROTTLE_CONFIG.enabled = false;
        RequestThrottler.logThrottle('请求频率控制已禁用');
      },
      
      // 设置最小请求间隔
      setMinInterval(ms) {
        REQUEST_THROTTLE_CONFIG.minInterval = ms;
        RequestThrottler.logThrottle(`最小请求间隔已设置为 ${ms}ms`);
      },
      
             // 获取当前状态
       getStatus() {
         return {
           enabled: REQUEST_THROTTLE_CONFIG.enabled,
           minInterval: REQUEST_THROTTLE_CONFIG.minInterval,
           queueLength: REQUEST_THROTTLE_CONFIG.pendingRequests.length,
           timeSinceLastRequest: Date.now() - REQUEST_THROTTLE_CONFIG.lastRequestTime,
           mergedRequests: REQUEST_THROTTLE_CONFIG.mergedRequests,
           version: REQUEST_THROTTLE_CONFIG.version
         };
      },
      
      // 清空请求队列（紧急情况使用）
      clearQueue() {
        const count = REQUEST_THROTTLE_CONFIG.pendingRequests.length;
        REQUEST_THROTTLE_CONFIG.pendingRequests = [];
        RequestThrottler.logThrottle(`已清空请求队列，共 ${count} 个请求`);
      }
    };

    // 初始启动日志
                   RequestThrottler.logThrottle(`商品卡片请求频率优化已加载 (最小间隔: ${REQUEST_THROTTLE_CONFIG.minInterval}ms，已启用去重功能)`);
